...
robj *tryObjectEncodingEx(robj *o, int try_trim) {
    ...
    /* 检查传递的类型为STRING，因为该方法仅支持对STRING类型进行编码。其他类型使用编码内存高效表示，但由实现该类型的命令处理*/
    serverAssertWithInfo(NULL, o, o->type == OBJ_STRING);

    /* 尝试仅对RAW或EMBSTR编码的对象进行一些专门的编码，换句话说，这些对象仍然由实际的字符数组表示
     * 对于非 Raw 和 Embstr，不进行编码处理
     */
    ...
    /* 对于共享的对象进行编码并不安全，共享对象可以在Redis的对象空间内的任意地方使用，并且可能在未处理他们的地方结束。
     *
     * 如果引用数量超过1，则代表对象是共享的，不进行编码
     */
    if (o->refcount > 1) return o;

    /* 检查内容长度是否能以long来表示，我们确信超过20个字符的字符串不能表示为32位或64位整数 */
    len = sdslen(s);
    // 处理小于20位的长度，并且解析正确，返回1
    if (len <= 20 && string2l(s, len, &value)) {
        ...
        /* 代表该对象可被编码为long，尝试使用一个共享对象。当maxmemory被使用的，避免用于共享整数，
         * 因为每个对象需要持有一个私有的LRU字段用于帮助LRU算法工作。
         */
...
struct redisMemOverhead *getMemoryOverheadData(void) {
    ...
    size_t frag_smallbins_bytes =
            server.cron_malloc_stats.allocator_frag_smallbins_bytes - server.cron_malloc_stats.
            lua_allocator_frag_smallbins_bytes;
    size_t allocated =
